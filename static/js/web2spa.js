...
var web2spa = {
	...
	***/
	...
	this.root_path = `/${this.app}/${this.mainctrl}/`;       // e.g. '/welcome/default/'
	this.start_path = `${this.root_path}${this.mainpage}/`;	// e.g. '/welcome/default/index/'
	this.static_path = `/${this.app}/static/`;               // e.g. '/welcome/static/'
	this.ajax_path = `/${this.app}/${settings.ajaxctrl || 'ajax'}/`;	// e.g. '/welcome/ajax/'
	...
	$(function() {	// waiting full application loading
	    ...
	    $('body').on('click', 'a[data-spa]', {add:true}, self.ajax_nav);
		...
		if (e.data.url.indexOf('logout')==-1) self.ajax_nav(e);  // all 'user/...' function performed via ajax, bot not 'logout'
	    });
    ...
    ***/
	...
	var path = `${this.app}/${opts.ctrl || this.mainctrl}`;
	if (opts.error_path) opts.master = true;
	if (!opts.master) path += `/${opts.page || this.mainpage}`;
	var route = path+':'+ctrl;
	...
	if (opts.error_path) this.error_path = `/${path}/${ctrl}/`;
	if (opts.shortcuts) {
	    path = path.split('/');
	    title = '';
	...
	var id = o.id;
	if (!this.cache[id]) this.add(id, document.getElementById(id).innerHTML);   // search template in current html document, if not found in cache
